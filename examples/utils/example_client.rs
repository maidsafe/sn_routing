...
use routing::{
    Authority, Client, ClientError, Event, ImmutableData, MessageId, MutableData, PublicIdExt,
    Response, SecretId, Value, XorName,
};
...
impl ExampleClient {
    ...
    pub fn put_mdata(&mut self, data: MutableData) -> Result<(), ClientError> {
        ...
        let tag = data.tag();
        let msg_id = MessageId::new();
        let requester = self.public_id().clone();
