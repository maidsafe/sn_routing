...
mod test {
    ...
    fn add() {
        ...
        let address1 : NameType = NameType::generate_random();
        let address2 : NameType = NameType::generate_random();
    ...
    fn add_single_value_quorum() {
        ...
        for i in 0..quorum_size-1 {
            assert!(accumulator.add(key, value, NameType::generate_random()).is_none());
        ...
        assert!(accumulator.add(key, value, NameType::generate_random()).is_some());
    ...
    fn add_multiple_values_quorum() {
        ...
        for _ in 0..quorum_size-1 {
            assert!(accumulator.add(key, rand::random::<u32>(), NameType::generate_random()).is_none());
            assert_eq!(accumulator.is_quorum_reached(key), false);
        }
        assert!(accumulator.add(key, rand::random::<u32>(), NameType::generate_random()).is_some());
        assert_eq!(accumulator.is_quorum_reached(key), true);
    }
    ...
    fn add_multiple_keys_quorum() {
        ...
        for _ in 0..quorum_size-1 {
            for noise_key in noise_keys.iter() {
                accumulator.add(noise_key.clone(), rand::random::<u32>(), NameType::generate_random());
            }
            assert!(accumulator.add(key, rand::random::<u32>(), NameType::generate_random()).is_none());
            assert_eq!(accumulator.is_quorum_reached(key), false);
        }
    ...
    fn delete() {
        ...
        let address : NameType = NameType::generate_random();
