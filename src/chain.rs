...
use block::{Block, PeersAndAge}; //, BlockState};
...
pub struct CurrentPeers {
    live_peers: BTreeSet<PeerId>, /// first group_size of these are the elders 
    ...
    fn add_vote(&mut self, vote: Vote, peer_id: &PeerId) -> Option<(NetworkEvent, PeersAndAge)> {
        ...
        for (pos, blk) in &mut self.blocks.iter_mut().enumerate() {
            if blk.payload() == vote.payload() {
                if blk.proofs().iter().any(|x| x.peer_id().pub_key() == peer_id.pub_key()) {
        ...
        if let Ok(ref mut blk) = Block::new(&vote, &peer_id) {
            ...
            return Some((blk.payload().clone(), PeersAndAge::new(1, peer_id.age() as usize)));
        }
