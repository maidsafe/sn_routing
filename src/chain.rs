...
impl DataChain {
    ...
    fn add_vote(
        ...
        let pub_key_matches = |x: &Proof| x.peer_info().sign_key() == peer_info.sign_key();
        ...
        if let Ok(ref mut blk) = Block::new(vote, peer_info) {
    ...
    fn validate_quorums(&self) -> bool {
        if let Some(mut prev) = self.blocks.first() {
            for blk in self.blocks.iter().skip(1) {
                ...
                if blk.get_peer_infos()
                    .intersection(&prev.get_peer_infos())
