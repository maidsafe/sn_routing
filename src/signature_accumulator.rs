...
mod tests {
    ...
    impl MessageAndSignatures {
        fn new<'a, I>(
            msg_sender_id: &SecretId,
            other_ids: I,
            all_ids: BTreeSet<PublicId>,
        ) -> MessageAndSignatures
        where
            I: Iterator<Item = &'a SecretId>,
        {
    ...
    struct Env {
        _msg_sender_id: SecretId,
        other_ids: Vec<SecretId>,
    ...
    impl Env {
        fn new() -> Env {
            let msg_sender_id = SecretId::new();
            let mut pub_ids = vec![msg_sender_id.public_id().clone()]
                .into_iter()
                .collect::<BTreeSet<_>>();
            let mut other_ids = vec![];
            for _ in 0..8 {
                let full_id = SecretId::new();
                let _ = pub_ids.insert(full_id.public_id().clone());
