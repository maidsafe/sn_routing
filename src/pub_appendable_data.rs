...
mod test {
                ...
                match PubAppendableData::new(identifier,
                    ...
                    Ok(mut new_pub_appendable_data) => {
                        assert_eq!(new_pub_appendable_data.add_signature(&attacker.1).ok(),
                                   Some(0));
                        assert!(new_pub_appendable_data
                        ...
                        assert!(orig_pub_appendable_data
                            .update_with_other(new_pub_appendable_data).is_err());
                    }
                    Err(error) => panic!("Error: {:?}", error),
                }
            }
    ...
    fn apply_wrapper() {
        ...
        let mut pub_appendable_data = unwrap!(PubAppendableData::new(identifier,
                                           0,
                                           vec![keys.0],
                                           vec![],
                                           BTreeSet::new(),
        ...
        let append_wrapper = AppendWrapper::new_pub(identifier, appended_data.clone(), 0);
        assert!(pub_appendable_data.apply_wrapper(append_wrapper));

        // apply wrapper with incorrect version
        let append_wrapper = AppendWrapper::new_pub(identifier, appended_data, 1);
        assert!(!pub_appendable_data.apply_wrapper(append_wrapper));
    }
}
