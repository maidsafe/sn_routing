...
impl SignedMessage {
    ...
    ///
    /// `min_group_size` is required when the message is from a _group_ (see doc on `Authority`).
    ///
...
mod tests {
    ...
    fn signed_message_check_integrity() {
        ...
        unwrap!(signed_message.check_integrity(min_group_size, None));
        ...
        assert!(signed_message.check_integrity(min_group_size, None).is_err());
