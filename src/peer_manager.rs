...
pub type SectionMap = BTreeMap<VersionedPrefix<XorName>, BTreeSet<PublicKeys>>;

#[derive(Default)]
pub struct PeerDetails {
    pub routing_peer_details: Vec<(PublicKeys, bool)>,
    pub out_of_sync_peers: Vec<PublicKeys>,
...
impl Peer {
    ...
    pub fn pub_id(&self) -> &PublicKeys {
        &self.pub_id
    }

    pub fn name(&self) -> XorName {
        self.pub_id.xor_name()
    }
...
pub struct PeerManager {
    connection_token_map: HashMap<u32, PublicKeys>,
    peers: HashMap<PublicKeys, Peer>,
    routing_table: RoutingTable<XorName>,
    our_pub_id: PublicKeys,
    candidate: Candidate,
...
impl PeerManager {
    ...
    pub fn new(
        ...
        PeerManager {
            ...
            peers: HashMap::new(),
            routing_table: RoutingTable::new(our_pub_id.xor_name(), min_section_size),
            our_pub_id,
    ...
    pub fn handle_bootstrap_request(&mut self, pub_id: &PublicKeys) {
    ...
    pub fn expect_candidate(&mut self, old_pub_id: PublicKeys) -> Result<(), RoutingError> {
    ...
    pub fn accept_as_candidate(
        ...
        old_pub_id: PublicKeys,
        target_interval: (XorName, XorName),
    ) -> (Prefix<XorName>, BTreeSet<PublicKeys>) {
    ...
    pub fn verified_candidate_info(&self) -> Result<(MessageContent, XorName), RoutingError> {
        ...
        Ok((
            MessageContent::CandidateApproval {
                new_pub_id: new_pub_id.clone(),
                new_client_auth: new_client_auth.clone(),
    ...
    pub fn handle_candidate_approval(
        ...
        if let Some(peer) = self.peers.get_mut(new_pub_id) {
            ...
            trace!("{} No peer with name {}", debug_id, new_pub_id.xor_name());
    ...
    pub fn handle_candidate_info(
        ...
        if new_pub_id.xor_name() < target_interval.0 || new_pub_id.xor_name() > target_interval.1 {
            warn!(
        ...
        let peer = match self.peers.get_mut(new_pub_id) {
            ...
            None => {
                ...
                return Err(RoutingError::UnknownConnection(new_pub_id.clone()));
            }
        };
    ...
    pub fn show_candidate_status(&self) {
        ...
        match self.candidate {
            ...
            Candidate::ResourceProof {
                ...
                challenge: None,
                ..
            } => trace!("{}{} is tunneling to us.", log_msg, new_pub_id.xor_name()),
            Candidate::ResourceProof {
                ref new_pub_id,
                ...
                log_msg = format!("{}{}", log_msg, new_pub_id.xor_name());
    ...
    pub fn add_to_routing_table(&mut self, pub_id: &PublicKeys) -> Result<(), RoutingError> {
        ...
        let peer = if let Some(peer) = self.peers.get_mut(pub_id) {
            ...
            return Err(RoutingError::UnknownConnection(pub_id.clone()));
        };
        ...
        let res = match self.routing_table.add(pub_id.xor_name()) {
    ...
    /// Splits the indicated section and returns the `PublicKeys`s of any peers to which we should not
    /// remain connected.
    pub fn split_section(
        &mut self,
        ver_pfx: VersionedPrefix<XorName>,
    ) -> (Vec<PublicKeys>, Option<Prefix<XorName>>) {
        ...
        let remove_candidate = match self.candidate {
            ...
            Candidate::ResourceProof { ref new_pub_id, .. } => {
                if !self
                    .routing_table
                    .our_prefix()
                    .matches(&new_pub_id.xor_name())
                {
                    Some(new_pub_id.clone())
                } else {
                    None
                }
            }
    ...
    pub fn add_prefix(&mut self, ver_pfx: VersionedPrefix<XorName>) -> Vec<PublicKeys> {
    ...
    pub fn merge_own_section(
        ...
        merge_version: u64,
        sections: SectionMap,
    ) -> (OwnMergeState<XorName>, Vec<PublicKeys>) {
        let needed = sections
            .iter()
            .flat_map(|(_, pub_ids)| pub_ids)
            .filter(|pub_id| !self.routing_table.has(&pub_id.xor_name()))
            .cloned()
            .collect();
    ...
    pub fn merge_other_section(
        ...
        let needed_names = self
            .routing_table
            .merge_other_section(ver_pfx, section.iter().map(PublicKeys::xor_name));
        section
            .iter()
            .filter(|id| needed_names.contains(&id.xor_name()))
            .cloned()
            .collect()
    }

    /// Returns `true` if we are directly connected to both peers.
    pub fn can_tunnel_for(&self, pub_id: &PublicKeys, dst_pub_id: &PublicKeys) -> bool {
        let peer_state = self.get_peer(pub_id).map(Peer::state);
        let dst_state = self.get_peer(dst_pub_id).map(Peer::state);
    ...
    pub fn remove_expired_peers(&mut self) -> Vec<PublicKeys> {
        let remove_candidate = if self.candidate.is_expired() {
            match self.candidate {
                ...
                | Candidate::AcceptedForResourceProof { ref old_pub_id, .. } => {
                    Some(old_pub_id.clone())
                }
                Candidate::ResourceProof { ref new_pub_id, .. } => Some(new_pub_id.clone()),
            }
        } else {
            None
        };
            ...
            .filter(|peer| match peer.state {
                ...
                | PeerState::Routing(RoutingConnection::Proxy(timestamp)) => {
                    if timestamp.elapsed() >= Duration::from_secs(JOINING_NODE_TIMEOUT_SECS) {
                        normalisable_conns.push(peer.pub_id().clone());
                    }
                    false
                }
                _ => peer.is_expired(),
    ...
    pub fn add_client_traffic(&mut self, pub_id: &PublicKeys, added_bytes: u64) {
    ...
    pub fn connected_to(&mut self, pub_id: &PublicKeys) {
        if let Some(peer) = self.peers.get_mut(pub_id) {
            match peer.state {
    ...
    pub fn tunnelling_to(&mut self, pub_id: &PublicKeys) -> bool {
    ...
    pub fn get_peer(&self, pub_id: &PublicKeys) -> Option<&Peer> {
        self.peers.get(pub_id)
    }

    /// Returns the given peer.
    pub fn get_peer_by_name(&self, name: &XorName) -> Option<&Peer> {
        let id = if let Some(id) = self.peers.keys().find(|id| &id.xor_name() == name) {
            id
        } else {
            return None;
        };
    ...
    pub fn set_peer_valid(&mut self, id: &PublicKeys, valid: bool) {
        if let Some(peer) = self.peers.get_mut(id) {
            peer.valid = valid;
        }
    }

    /// Returns the `PublicKeys` of the node with a given name.
    pub fn get_pub_id(&self, name: &XorName) -> Option<&PublicKeys> {
        self.get_peer_by_name(name).map(Peer::pub_id)
    }

    /// Returns the `PublicKeys`s of nodes bearing the names.
    pub fn get_pub_ids(&self, names: &BTreeSet<XorName>) -> BTreeSet<PublicKeys> {
        names
            .iter()
            .filter_map(|name| {
                if name == &self.our_pub_id.xor_name() {
                    Some(&self.our_pub_id)
                } else {
    ...
    /// Returns all syncing peer's `PublicKeys`s, names and whether connected via a tunnel or not;
    /// together with all out-of-sync peer's `PublicKeys`s.
    ...
    pub fn get_routing_peer_details(&mut self) -> PeerDetails {
            ...
            match self.get_peer_by_name(name) {
                ...
                Some(peer) => {
                    if !peer.is_routing() {
                        ...
                        result.out_of_sync_peers.push(peer.pub_id.clone());
                    }
                }
            };
        }
        ...
        for peer in self.peers.values() {
            let is_tunnel = match peer.state {
                PeerState::Routing(conn) => {
                    if !self.routing_table.has(&peer.name()) {
                        nodes_missing_from_rt.push(peer.pub_id.clone());
                        continue;
                    } else {
            ...
            result
                ...
                .push((peer.pub_id.clone(), is_tunnel));
        }
    ...
    pub fn correct_state_to_direct(&mut self, pub_id: &PublicKeys) {
    ...
    pub fn correct_state_to_tunnel(&mut self, pub_id: &PublicKeys) {
    ...
    pub fn is_potential_tunnel_node(&self, tunnel_name: &XorName, client_name: &XorName) -> bool {
        if &self.our_pub_id.xor_name() == tunnel_name || &self.our_pub_id.xor_name() == client_name
            || !self
    ...
    pub fn set_searching_for_tunnel(&mut self, pub_id: PublicKeys, valid: bool) -> Vec<PublicKeys> {
        ...
        self.routing_table
            .iter()
            .filter(|tunnel_name| self.is_potential_tunnel_node(tunnel_name, &pub_id.xor_name()))
            .filter_map(|name| self.get_peer_by_name(name))
            .map(Peer::pub_id)
            .cloned()
            .collect()
    ...
    pub fn connection_info_prepared(
        ...
        let infos = match opt_their_info {
            Some((their_info, msg_id)) => {
                ...
                self.insert_peer(Peer::new(pub_id.clone(), state, valid, reconnecting));
    ...
    pub fn get_connection_token(
        ...
        let token = rand::random();
        let _ = self.connection_token_map.insert(token, pub_id.clone());
        self.insert_peer(Peer::new(
            pub_id,
            PeerState::ConnectionInfoPreparing {
                us_as_src: src,
    ...
    pub fn peers_needing_tunnel(&self) -> Vec<PublicKeys> {
        self.peers
            .values()
            .filter_map(|peer| match peer.state {
                PeerState::SearchingForTunnel => Some(peer.pub_id.clone()),
                _ => None,
            })
            .collect()
    }
    ...
    pub fn insert_peer(&mut self, peer: Peer) {
        let _ = self.peers.insert(peer.pub_id.clone(), peer);
    }
    ...
    pub fn remove_peer(
        ...
        let remove_candidate = match self.candidate {
            ...
            Candidate::ResourceProof { ref new_pub_id, .. } => new_pub_id == pub_id,
        };
    ...
    /// Removes the peer with the given ID if present, and returns such `PublicKeys`s.
    /// If the peer is also our proxy, or we are theirs,
    /// it is reinserted as a proxy or joining node.
    fn remove_split_peers(&mut self, ids: Vec<PublicKeys>) -> Vec<PublicKeys> {
        {
            ...
            for peer in filtered_peers {
                if self
                    .routing_table
                    .need_to_add(&peer.pub_id.xor_name())
                    .is_err()
                {
                    peer.valid = false;
                }
    ...
    pub fn ideal_rt(&self) -> SectionMap {
        ...
        for pub_id in self
            ...
            .chain(iter::once(&self.our_pub_id))
        {
            if let Some(versioned_prefix) = versioned_prefixes
                .iter()
                .find(|versioned_prefix| versioned_prefix.prefix().matches(&pub_id.xor_name()))
            {
                let _ = result
...
mod tests {
    ...
    pub fn connection_info_prepare_receive() {
        ...
        let our_pub_id = SecretKeys::new().public_keys().clone();
        let their_pub_id = SecretKeys::new().public_keys().clone();
        let mut peer_mgr = PeerManager::new(min_section_size, our_pub_id.clone(), false);
