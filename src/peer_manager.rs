...
impl PeerManager {
    ...
    pub fn add_to_routing_table(&mut self,
        ...
        if result.is_some() {
            let state = PeerState::Connected(match self.node_map.remove(public_id.name()) {
    ...
    pub fn can_tunnel_for(&self, peer_id: &PeerId, dst_id: &PeerId) -> bool {
        ...
        match (peer_state, dst_state) {
            (Some(&PeerState::Connected(false)), Some(&PeerState::Connected(false))) => true,
            _ => false,
        }
    }
    ...
    pub fn get_routing_peer(&self, peer_id: &PeerId) -> Option<&PublicId> {
        self.pub_id_map.get(peer_id).and_then(|pub_id| {
            if let Some(&PeerState::Connected(_)) = self.get_state(pub_id) {
                Some(pub_id)
            } else {
                None
            }
    ...
    /// Returns empty vector of candidates if it is already in Connected state.
    ...
    pub fn connection_info_received(&mut self,
        ...
        match self.node_map.remove(pub_id.name()) {
            ...
            Some((timestamp, PeerState::Connected(tunnel))) => {
                // TODO: We _should_ retry connecting if the peer is connected via tunnel.
                let _ = self.node_map
                    .insert(*pub_id.name(), (timestamp, PeerState::Connected(tunnel)));
