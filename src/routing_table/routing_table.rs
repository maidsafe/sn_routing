...
use super::xor_name::{XOR_NAME_LEN, XorName};
...
impl Prefix {
    pub fn new(valid_bit_count: usize, address: XorName) -> Self {
        Prefix {
    ...
    pub fn is_belong_to(&self, name: &XorName) -> bool {
    ...
    pub fn max_bucket_index(&self, name: &XorName) -> usize {
        self.address.max_bucket_index(name)
    }
}

impl fmt::Binary for Prefix {
...
impl CloseGroup {
    pub fn new(min_group_size: usize, prefix: Prefix, members: Vec<XorName>) -> Self {
        CloseGroup {
    ...
    pub fn get_members(&self) -> &Vec<XorName> {
        &self.members
    }
    ...
    pub fn add(&mut self, member: XorName) -> bool {
    ...
    pub fn remove(&mut self, name: &XorName) -> bool {
        match self.search(name) {
            Ok(index) => {
    ...
    pub fn update(&mut self, member: XorName) -> bool {
        match self.search(&member) {
            Ok(index) => {
    ...
    pub fn get(&self, name: &XorName) -> Option<&XorName> {
        match self.search(name) {
    ...
    pub fn split(&mut self) -> Option<CloseGroup> {
    ...
    fn search(&self, name: &XorName) -> Result<usize, usize> {
        self.members.binary_search_by(|member| member.cmp(name))
    }
}

impl fmt::Binary for CloseGroup {
    fn fmt(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
...
pub struct RoutingTable {
    ...
    buckets: Vec<(Prefix, Vec<CloseGroup>)>,
}
...
impl RoutingTable {
    ...
    // disconnect.
    pub fn new(our_info: XorName, min_group_size: usize) -> Self {
        RoutingTable {
            our_info: our_info,
            min_group_size: min_group_size,
            buckets: vec![],
    ...
    pub fn allow_to_add(&self, name: &XorName) -> bool {
    ...
    pub fn is_close(&self, name: &XorName, _group_size: usize) -> bool {
        if self.buckets.len() == 0 {
            true
        } else {
    ...
    pub fn closest_nodes_to(&self, target: &XorName, n: usize, ourselves: bool) -> Vec<XorName> {
    ...
    pub fn target_nodes(&self, dst: Destination, hop: &XorName, route: usize) -> Vec<XorName> {
    ...
    pub fn other_close_nodes(&self, name: &XorName, group_size: usize) -> Option<Vec<XorName>> {
    ...
    fn search(&self, name: &XorName) -> (Result<usize, usize>, Result<usize, usize>) {
...
impl fmt::Binary for RoutingTable {
    fn fmt(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
        try!(writeln!(formatter,
