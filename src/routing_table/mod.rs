...
pub use self::prefix::{Prefix, PrefixCollection, PrefixMap, PrefixSet};
...
impl RoutingTable {
    ...
    pub fn section_with_prefix(&self, prefix: &Prefix) -> Option<&BTreeSet<XorName>> {
    ...
    pub fn split(&mut self, prefix: Prefix) -> (Vec<XorName>, Option<Prefix>) {
        ...
        let prefix0 = prefix.pushed(false).with_incremented_version();
        let prefix1 = prefix.pushed(true).with_incremented_version();
        let (section0, section1) = to_split.into_iter().partition::<BTreeSet<_>, _>(
            |name| prefix0.matches(name),
        );

        for (prefix, section) in vec![(prefix0, section0), (prefix1, section1)] {
            if self.our_prefix.is_neighbour(&prefix) {
                self.insert_new_section(prefix, section);
            } else {
    ...
    pub fn should_merge(&self) -> bool {
        ...
        if bit_count == 0 ||
            !self.sections.contains_unversioned(
                &self.our_prefix.sibling(),
            )
        {
        ...
        I: IntoIterator<Item = XorName>,
    {
        ...
        self.sections.get(&prefix).map_or_else(
            BTreeSet::new,
            |section| {
                members
                    .into_iter()
    ...
    fn split_our_section(&mut self, version: u64) -> Vec<XorName> {
        if self.our_prefix.version() != version {
            debug!(
                "{:?} Not splitting our section with {:?}, \
        ...
        let other_prefix = self.our_prefix.pushed(!next_bit).with_incremented_version();
        self.our_prefix = self.our_prefix.pushed(next_bit).with_incremented_version();
    ...
    fn insert_new_section(&mut self, prefix: Prefix, section: BTreeSet<XorName>) {
        // Check if we already have a section with the same prefix bits, but
        // possibly different version.
        let section = if let Some(old_prefix) = self.sections.find_unversioned(&prefix).map(
            |(old_prefix, old_section)| {
                error!(
    ...
    fn merge(&mut self, new_prefix: &Prefix) {
        let checker = |pfx: &Prefix| new_prefix.is_extension_of(pfx);
        if new_prefix.is_extension_of(&self.our_prefix) || self.sections.keys().any(checker) {
            return; // Not a merge!
        }
        let dropped_names = self.add_prefix(*new_prefix);
        if !dropped_names.is_empty() {
            error!(
    ...
    pub fn check_invariant(
        ...
        if self.sections.contains_unversioned(&self.our_prefix) {
            return warn(format!(
