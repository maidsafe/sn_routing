...
impl Timer {
    ...
    /// Schedule a timeout event after `duration`.  Returns a token that can be used to identify the
    /// timeout event.
    pub fn schedule(&mut self, duration: Duration) -> u64 {
        ...
        // Assumes the granularity of the clock won't allow multiple events to be added with the
        // same timestamps.
        let _ = detail.deadlines.insert(Instant::now() + duration, token);
        cond_var.notify_one();
        token
    }
    ...
    fn run(sender: RoutingActionSender, detail_and_cond_var: Arc<(Mutex<Detail>, Condvar)>) {
        ...
        while !detail.cancelled {
            ...
            for expired in expired_list {
                ...
                let token = detail.deadlines.remove(&expired).expect("Bug in `BTreeMap`.");
    ...
    fn schedule() {
        ...
        };
        {
            ...
            for i in 0..count {
                ...
                let category = category_receiver.try_recv();
                match unwrap_result!(category.clone()) {
                ...
                let action = action_receiver.try_recv();
                match unwrap_result!(action.clone()) {
                    Action::Timeout(token) => assert_eq!(token, (count - i - 1) as u64),
                    _ => {
                        panic!("Expected `Action::Timeout`, but received {:?}", action);
                    }
                }
            }
