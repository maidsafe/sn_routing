...
mod test {
  ...
  fn handle_put_get() {
    ...
    let data_name = DhtId::new(&sdv.get_name().0);
    ...
    match get_result.ok().unwrap() {
      ...
      routing::Action::Reply(x) => {
        ...
        assert_eq!(sdv_after.get_name().0.to_vec(), [3u8; 64].to_vec());
        assert_eq!(sdv_after.get_value().len(), 0);
        // assert_eq!(sdv_after.get_value()[0].len(), 1);
        // assert_eq!(sdv_after.get_value()[0][0].0.to_vec(), [5u8; 64].to_vec());
      }
    }
  }
}
