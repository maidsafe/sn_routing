...
//                cache_options: ::data_cache_options::DataCacheOptions::new(),
    ...
    fn extract_original_signed_request(response: ::messages::Content) -> Result<::messages::SignedRequest,
                                                                                ::error::RoutingError> {
        match response {
            ::messages::Content::InternalResponse(internal_response) => {
                match internal_response {
                    ::messages::InternalResponse::GetPublicId { signed_request, .. } => signed_request,
                    ::messages::InternalResponse::GetPublicIdWithEndpoints { signed_request, .. } => signed_request,
                    ::messages::InternalResponse::RelocatedNetworkName { signed_request, .. } => signed_request,
                }
            },
            _ => Err(::error::RoutingError::InvalidMessage),
        }
    }
    ...
    fn handle_routing_message(&mut self, signed_message: SignedMessage) -> RoutingResult {
        ...
        let message = try!(signed_message.get_routing_message());
        let public_sign_key = signed_message.signing_public_key();
        ...
        let (accumulated_message, orig_signed_request) = if message.from_authority.is_group() {
            match self.accumulate(&routing_message) {
                Some(output_message) => {
                    output_message, RoutingNode::extract_original_signed_request(routing_message.content).
                },
                None => {
            ...
            Content::InternalRequest(request) => {
                match request {
                    InternalRequest::Endpoints { encrypted_endpoints, nonce_bytes } => {
                        match opt_token {
                            // TODO to_authority needs validation
                            Some(signed_request) => self.handle_endpoints(encrypted_endpoints,
                                                                          nonce_bytes,
                    ...
                    InternalRequest::Relocate { current_public_id, } => {
                        match opt_token {
                            Some(signed_request) =>
                                self.handle_relocate_request(current_public_id,
                                                             accumulated_message.from_authority,
                                                             accumulated_message.to_authority,
                                                             signed_request),
                            None => return Err(RoutingError::UnknownMessageType),
                    ...
                    InternalRequest::Connect => {
                        match opt_token {
                            Some(_signed_request) => {
                                if let ::authority::Authority::ManagedNode(name) = accumulated_message.from_authority {
                                    self.handle_connect_request(name)
                                } else {
                                    return Err(RoutingError::BadAuthority)
                                }
                            },
                        ...
                        match opt_token {
                            Some(signed_request) => {
                                if let (::authority::Authority::ManagedNode(from_name),
                                    ...
                                    self.handle_get_public_id(from_name, to_name, signed_request)
                                } else {
                                    return Err(RoutingError::BadAuthority)
                                }
                            },
    ...
    fn accumulate(&mut self,
                  ...
                  public_sign_key: ::sodiumoxide::crypto::box_::PublicKey) -> Option<RoutingMessage> {
        ...
        let dynamic_quorum_size = if let Content::InternalResponse(InternalResponse::RelocatedNetworkName(..)) =
                                         message.content {
            ...
            (&Authority::Client(_bootstrap_node, key), &Authority::NaeManager(name)) => {
                ...
                // From X -> Y; Send to close group of the relocated name
                {
                    let request = ::messages::InternalRequest::ExpectCloseNode {
                        expect_public_id: their_public_id.clone(),
                    };

                    let routing_message = RoutingMessage {
                        from_authority: to_authority.clone(),
                        to_authority: Authority::NodeManager(relocated_name),
    ...
    // Signed request here is the signed request of the original sender (one trying to relocate)
    ...
    //     let _ = self.node_id_cache.insert(relocated_id.name().clone(), relocated_id.clone());
    ...
    fn handle_relocation_response(&mut self,
        ...
        match message.content {
            Content::InternalRequest(InternalRequest::RequestNetworkName(ref original_public_id)) => {
                ...
                let from_authority = try!(self.get_client_authority());

                // Send connect request as a client
                for peer_id in close_group_ids {
                    try!(self.send_endpoints(&peer_id,
                                             from_authority.clone(),
                                             ::authority::Authority::ManagedNode(peer_id.name().clone())));
                    if let Some(prev_id) = self.node_id_cache.insert(peer_id.name().clone(), peer_id) {
    ...
    fn send_endpoints(&mut self,
                      ...
                      to_authority: ::authority::Authority) -> RoutingResult {
        ...
        let encrypted_endpoints = ::sodiumoxide::crypto::box_::seal(&encoded_endpoints,
                                                                    &nonce,
    ...
    fn handle_endpoints(&mut self,
                        ...
                        from_authority: ::authority::Authority,
                        to_authority: ::authority::Authority,
        ...
        match self.node_id_cache.retrieve_all().iter().find(|elt| *elt.1.signing_public_key() == client_key) {
            Some(&(ref name, ref their_public_id)) => {
                if self.routing_table.want_to_add(&name) {
                    ...
                    self.send_endpoints(their_public_id, to_authority, from_authority)
                } else {
                    debug!("{}No longer want to connect to relocating node although present in our \
                           node_id_cache", self.us());

                    Err(RoutingError::RefusedFromRoutingTable)
                }
            },
            None => {
                debug!("{}Key was not previously cached. \
                       Ignoring Endpoint request from unknown relocating node", self.us());
    ...
    fn send_connect_request(&mut self, peer_name: &NameType) -> RoutingResult {
        ...
        let routing_message = ::messages::RoutingMessage {
            from_authority: ::authority::Authority::ManagedNode(self.full_id.public_id().name().clone()),
            to_authority: ::authority::Authority::ManagedNode(peer_name.clone()),
            content: ::messages::Content::InternalRequest(request),
            group_keys: None,
        };
    ...
    fn handle_connect_request(&mut self, name: ::NameType) -> RoutingResult {
        ...
        let routing_message = ::messages::RoutingMessage {
            from_authority: ::authority::Authority::ManagedNode(self.full_id.public_id().name().clone()),
            to_authority: ::authority::Authority::NodeManager(name),
            content: ::messages::Content::InternalRequest(request),
            group_keys: None,
        };
    ...
    fn handle_get_public_id_response(&mut self,
        ...
        try!(self.send_endpoints(&public_id,
                                 orig_routing_message.from_authority,
    ...
    fn handle_get_public_id_with_endpoints_response(&mut self,
        ...
        let serialised_endpoints = try!(::sodiumoxide::crypto::box_::open(&encrypted_endpoints,
                                                                          &nonce,
                                                                          public_id.encrypting_public_key(),
                                                                          self.full_id.encrypting_private_key()))
