...
/// DHT node
pub struct RoutingNode<F: Interface, G: CreatePersonas> {
    genesis: Box<G>,
    id: types::Id,
    own_name: NameType,
...
impl<F, G> RoutingNode<F, G> where F: Interface, G : CreatePersonas {
    pub fn new(genesis) -> RoutingNode<F, G> {
...
//      self.send(self.routing_table.target_nodes(target).iter().filter_map(|node_info|self.name_to_endpoint(node_info.id)), msg);
    ...
    fn run_membrane(&mut self) {
        let mut membrane = RoutingMembrane::new(self.interface);
        self.our_membrane = Some(spawn(move || membrane.run()));
    }
}
