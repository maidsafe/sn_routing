...
use crate::{
    ...
    rng::{self, MainRng},
    section::{
        self, EldersInfo, SectionKeyShare, SectionKeysProvider, SectionProofChain, SharedState,
    },
};
...
impl Env {
    ...
    fn perform_elders_change(&mut self) {
        ...
        let full_ids = self.elders.drain(..).map(|elder| elder.full_id).collect();
...
fn handle_elders_update_on_parsec_prune() {
    ...
    env.send_elders_update(
        &full_ids[0],
        elders_info,
        1,
        SectionProofChain::new(env.public_key()),
    )
    .unwrap();
...
fn handle_elders_update_on_elders_change() {
    ...
    env.send_elders_update(&full_ids[0], elders_info, 1, proof_chain)
        .unwrap();
...
fn handle_elders_update_ignore_old_parsec_vesions() {
    ...
    env.send_elders_update(&full_ids[0], elders_info.clone(), 1, proof_chain.clone())
        .unwrap();
    assert_eq!(env.subject.parsec_last_version(), 1);

    env.send_elders_update(&full_ids[0], elders_info.clone(), 2, proof_chain.clone())
        .unwrap();
...
fn untrusted_elders_update() {
    ...
    env.sk_set = new_sk_set;
    env.send_elders_update(&full_ids[0], elders_info, 1, SectionProofChain::new(new_pk))
        .unwrap();
