...
impl RoutingCore {
    ...
    /// Returns the available Boostrap connections as Peers. If we are a node,
    ...
    pub fn bootstrap_endpoints(&self) -> Option<Vec<Peer>> {
        ...
        if self.is_node() { return None };
        Some(self.relay_map.bootstrap_connections())
    }

    /// Returns true if bootstrap connections are available. If we are a node,
