...
use {CrustEvent, PrivConnectionInfo, PubConnectionInfo, Service, QUORUM_DENOMINATOR,
     QUORUM_NUMERATOR};
...
use messages::{DirectMessage, HopMessage, Message, MessageContent, RoutingMessage, SectionList,
               SignedMessage, UserMessage, UserMessageCache, DEFAULT_PRIORITY, MAX_PARTS,
               MAX_PART_LEN};
...
impl Peer {
    ...
    fn handle_new_message(
        ...
        match serialisation::deserialise(&bytes) {
            ...
            Ok(Message::TunnelHop { content, src, dst }) => if dst == *self.full_id.public_id() {
    ...
    fn check_direct_message_sender(
        ...
        match self.peer_mgr.get_peer(pub_id).map(PeerInfo::state) {
    ...
    fn handle_hop_message(
        ...
        let mut hop_name_result = match self.peer_mgr.get_peer(&pub_id).map(PeerInfo::state) {
            Some(&PeerState::Bootstrapper { .. }) => {
                warn!(
    ...
    fn handle_tunnel_closed(&mut self, src_id: PublicId, dst_id: PublicId, outbox: &mut EventBox) {
        if self.tunnels.remove(dst_id, src_id) {
            ...
            if !self.crust_service.is_connected(&dst_id) {
                let _fixme = self.dropped_peer(&dst_id, outbox, true);
            }
        }
    }
    ...
    fn disconnect_peer(&mut self, pub_id: &PublicId, outbox: Option<&mut EventBox>) {
        if self.peer_mgr.is_routing_peer(pub_id) {
            ...
            match outbox {
                ...
                None => if self.tunnels.is_tunnel_node(pub_id) {
                    debug!(
    ...
    fn handle_relocate_request(
        ...
        let relocation_dst = self.next_relocation_dst
    ...
    fn handle_own_section_merge(
        ...
        if let Some(our_merged_section) = self.our_merged_section
        ...
        loop {
            ...
            if let Some(merged_section) = self.our_merged_section
    ...
    fn purge_invalid_rt_entries(&mut self, outbox: &mut EventBox) -> Transition {
        ...
        for pub_id in peer_details.out_of_sync_peers {
            let _fixme = self.crust_service.disconnect(&pub_id);
            let _fixme = self.dropped_peer(&pub_id, outbox, true);
        }
        ...
        for (pub_id, is_tunnel) in peer_details.routing_peer_details {
            if is_tunnel {
                match self.tunnels.tunnel_for(&pub_id) {
                    Some(tunnel_node_id) => if !self.crust_service.is_connected(tunnel_node_id) {
                        log_or_panic!(
                            LogLevel::Debug,
                    ...
                    None => if self.crust_service.is_connected(&pub_id) {
                        log_or_panic!(
                            LogLevel::Debug,
    ...
    fn get_signature_target(&self, src: &Authority, route: u8) -> Option<XorName> {
        ...
        let list: Vec<&XorName> = match *src {
            ...
            Section(_) => self.routing_table()
                .our_section()
                .iter()
                .sorted_by(|&lhs, &rhs| src.name().cmp_distance(lhs, rhs)),
            PrefixSection(ref pfx) => self.routing_table()
                .iter()
    ...
    fn get_targets(
        ...
        if self.is_proper() && !force_via_proxy {
            ...
            let new_sent_to = if self.in_authority(&routing_msg.dst) {
                sent_to
                    .iter()
                    .chain(targets.iter().filter(
                        |target| match self.peer_mgr.get_peer_by_name(target).map(PeerInfo::state) {
...
impl Bootstrapped for Peer {
    ...
    fn send_routing_message_via_route(
        ...
        let sending_names = match routing_msg.src {
            ...
            PrefixSection(ref prefix) => self.routing_table()
                .all_sections()
                .into_iter()
        ...
        match self.get_signature_target(&signed_msg.routing_message().src, route) {
            ...
            Some(target_name) => if let Some(&pub_id) = self.peer_mgr.get_pub_id(&target_name) {
