...
impl JoiningNode {
    ...
    fn compute_new_id(&self, name_range: &(XorName, XorName)) -> Result<PublicId, RoutingError> {
        let now = Instant::now();
        loop {
    ...
    fn dispatch_routing_message(&mut self,
        ...
        match (msg_content, msg_src, msg_dst) {
            ...
            (MessageContent::GetNameRangeResponse { name_range, close_group_ids, .. },
             ...
             dst) => {
			    let relocated_id = try!(self.compute_new_id(&name_range));
				Ok(self.handle_get_name_range_response(relocated_id, close_group_ids, dst))
			}
            // ConnectionInfo
