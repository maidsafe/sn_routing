...
pub trait Base: Debug {
    ...
    fn id(&self) -> &PublicInfo {
        self.full_info().public_info()
    }

    fn name(&self) -> XorName {
        self.full_info().public_info().name()
    }
    ...
    fn send_message(&mut self, pub_info: &PublicInfo, message: Message) {
    ...
    // Sends the given `bytes` to the peer with the given Crust `PublicInfo`. If that results in an
    // error, it disconnects from the peer.
    fn send_or_drop(&mut self, pub_info: &PublicInfo, bytes: Vec<u8>, priority: u8) {
        self.stats().count_bytes(bytes.len());

        if let Err(err) = self.crust_service().send(pub_info, bytes, priority) {
            info!("{:?} Connection to {} failed: {:?}", self, pub_info, err);
