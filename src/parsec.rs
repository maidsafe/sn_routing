...
impl ParsecMap {
    ...
    pub fn handle_request(
        ...
        let response = parsec
            ...
            .map(|response| MessageContent::ParsecResponse(msg_version, response))
            .map_err(|err| {
    ...
    pub fn create_gossip(
        ...
        version: u64,
        target: &id::PublicId,
    ) -> Result<MessageContent, CreateGossipError> {
        let request = self
            .map
            .get_mut(&version)
            .ok_or(CreateGossipError::MissingVersion)?
            .create_gossip(target)?;
        Ok(MessageContent::ParsecRequest(version, request))
    }
