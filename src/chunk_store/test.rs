...
mod test {
    ...
    fn add_random_name_value_pairs(number: usize, size: usize) -> NameValueContainer {
        ...
        loop {
            container.push((::utils::random_name(), get_random_non_empty_string(size)));
    ...
    impl ChunkStoreTest {
        pub fn new() -> ChunkStoreTest {
            ChunkStoreTest {
                chunk_store: ::chunk_store::ChunkStore::new(K_DEFAULT_MAX_DISK_USAGE).unwrap(),
        ...
        pub fn populate_chunk_store(&mut self,
            ...
            self.chunk_store = ::chunk_store::ChunkStore::new(disk_usage).unwrap();
    ...
    fn successful_store() {
        ...
        let mut chunk_store = ::chunk_store::ChunkStore::new(k_disk_size).unwrap();
    ...
    fn should_fail_on_disk_overfill() {
        ...
        let name_value_container = chunk_store_utest.populate_chunk_store(num_entries,
                                                                          num_disk_entries)
                                                    .0;
        let name = ::utils::random_name();
