...
impl TestNode {
    fn build(network: &Network) -> TestNodeBuilder {
        TestNodeBuilder {
            network: network,
            first_node: false,
            config: None,
...
fn create_connected_nodes_with_cache(network: &Network,
    ...
    nodes.push(TestNode::build(network)
                        .first()
                        .endpoint(Endpoint(0))
                        .cache(use_cache)
                        .unwrap());
                        ...
                        nodes: &mut Vec<TestNode>) -> Option<usize>
{
    ...
    if len > GROUP_SIZE + 2 && rng.gen_weighted_bool(3) {
        ...
        nodes.insert(index, TestNode::build(network).config(config).unwrap());
        Some(index)
    }
}
...
struct TestCache(RefCell<HashMap<DataIdentifier, Response>>);
...
impl Cache for TestCache {
    fn get(&self, request: &Request) -> Option<Response> {
        if let Request::Get(identifier, _) = *request {
            self.0.borrow().get(&identifier).cloned()
        } else {
            None
        }
    }
    ...
    fn put(&self, response: Response) {
        let identifier = if let Response::GetSuccess(ref data, _) = response {
            data.identifier()
        } else {
            return;
        };

        let _ = self.0.borrow_mut().insert(identifier, response);
    }
}
...
fn node_joins_in_front() {
    ...
    let config = Config::with_contacts(&[nodes[0].handle.endpoint()]);
    nodes.insert(0, TestNode::build(&network).config(config).unwrap());
...
fn multiple_joining_nodes() {
    ...
    nodes.insert(0, TestNode::build(&network).config(config.clone()).unwrap());
    nodes.insert(0, TestNode::build(&network).config(config.clone()).unwrap());
    nodes.push(TestNode::build(&network).config(config.clone()).unwrap());
...
fn response_caching() {
    ...
    // Second request should hit the cache, thus the nodes should not raise any
    // request events.
    let message_id = MessageId::new();
