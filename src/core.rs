...
impl Core {
    ...
    pub fn new(event_sender: mpsc::Sender<Event>,
        ...
        let mut core = Core {
            ...
            peer_mgr: PeerManager::new(our_name),
    ...
    fn handle_connect_success(&mut self, peer_id: PeerId) {
        ...
        } else if let Some(pub_id) = self.peer_mgr.get_communicating_peer(&peer_id) {
    ...
    fn find_tunnel_for_peer(&mut self, peer_id: PeerId, pub_id: &PublicId) {
        for (name, peer_id) in self.peer_mgr.set_searching_for_tunnel(peer_id, pub_id) {
    ...
    fn handle_hop_message(&mut self,
        ...
        let hop_name;
        if self.state == State::Node {
            if let Some(public_id) = self.peer_mgr.get_communicating_peer(&peer_id) {
    ...
    fn handle_direct_message(&mut self,
        ...
        match direct_message {
            ...
            DirectMessage::ClientToNode => {
                ...
                if self.peer_mgr.get_communicating_peer(&peer_id).is_none() {
    ...
    fn disconnect_peer(&mut self, peer_id: &PeerId) {
        if let Some(&pub_id) = self.peer_mgr.get_communicating_peer(peer_id) {
    ...
    fn set_self_node_name(&mut self, new_name: XorName) {
        ...
        self.peer_mgr.reset_routing_table(new_name);
    }
    ...
    fn dropped_tunnel_node(&mut self, peer_id: &PeerId) {
        let peers = self.tunnels
            ...
            .into_iter()
            .filter_map(|dst_peer_id| {
                match self.peer_mgr.get_communicating_peer(&dst_peer_id) {
                    Some(dst_pub_id) => Some((dst_peer_id, *dst_pub_id)),
                    None => None,
                }
            })
            .collect_vec();
        for (dst_peer_id, dst_pub_id) in peers {
            self.dropped_routing_node_connection(&dst_peer_id);
            debug!("{:?} Lost tunnel for peer {:?} ({:?}). Requesting new tunnel.",
                   self,
                   dst_peer_id,
                   dst_pub_id.name());
            self.find_tunnel_for_peer(dst_peer_id, &dst_pub_id);
        }
    }
