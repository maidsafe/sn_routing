...
use safe_crypto::{PublicKeys, SecretKeys, Signature};
use sha3::Digest256;
...
pub enum DirectMessage {
    ...
    SectionListSignature(SectionList, Signature),
    ...
    CandidateInfo {
        /// `PublicKeys` from before relocation.
        old_pub_id: PublicKeys,
        /// `PublicKeys` from after relocation.
        new_pub_id: PublicKeys,
        /// Signature of concatenated `PublicKeys`s using the pre-relocation key.
        signature_using_old: Signature,
        /// Signature of concatenated `PublicKeys`s and `signature_using_old` using the
        /// post-relocation key.
...
impl HopMessage {
    ...
    pub fn verify(&self, verification_key: &PublicKeys) -> Result<(), RoutingError> {
...
pub struct SectionList {
    ...
    // TODO(MAID-1677): pub signatures: BTreeSet<(PublicKeys, Signature)>,
    pub_ids: BTreeSet<PublicKeys>,
}

impl SectionList {
    /// Create
    pub fn new(prefix: Prefix<XorName>, pub_ids: BTreeSet<PublicKeys>) -> Self {
        SectionList { prefix, pub_ids }
    }

    /// Create from any object convertable to an iterator
    pub fn from<I: IntoIterator<Item = PublicKeys>>(prefix: Prefix<XorName>, pub_ids: I) -> Self {
...
pub struct SignedMessage {
    ...
    signatures: BTreeMap<PublicKeys, Signature>,
}

impl SignedMessage {
    /// Creates a `SignedMessage` with the given `content` and signed by the given `secret_id`.
    ///
    /// Requires the list `src_sections` of nodes who should sign this message.
    pub fn new(
        content: RoutingMessage,
        secret_id: &SecretKeys,
        mut src_sections: Vec<SectionList>,
    ) -> Result<SignedMessage, RoutingError> {
        src_sections.sort_by_key(|list| list.prefix);
        let sig = secret_id.sign_detached(&serialise(&content)?);
        Ok(SignedMessage {
            content,
            src_sections,
            signatures: iter::once((secret_id.public_keys().clone(), sig)).collect(),
        })
    }
    ...
    pub fn signed_by(&self, pub_id: &PublicKeys) -> bool {
    ...
    /// list.
    pub fn add_signature(&mut self, pub_id: PublicKeys, sig: Signature) {
    ...
    fn is_sender(&self, pub_id: &PublicKeys) -> bool {
        self.src_sections
            .iter()
    ...
    // valid).
    fn find_invalid_sigs(&self, signed_bytes: Vec<u8>) -> Vec<PublicKeys> {
        let invalid = self
            .signatures
            .iter()
            .filter_map(|(pub_id, sig)| {
                // Remove if not in sending nodes or signature is invalid:
                let is_valid = if let Authority::Client {
                    ref client_pub_id, ..
                } = self.content.src
                {
                    client_pub_id == pub_id && client_pub_id.verify_detached(sig, &signed_bytes)
                } else {
                    self.is_sender(pub_id) && pub_id.verify_detached(sig, &signed_bytes)
                };
                if is_valid {
                    None
                } else {
        ...
        match self.content.src {
            ClientManager(_) | NaeManager(_) | NodeManager(_) => {
                ...
                let valid_names: HashSet<_> = self
                    .src_sections
                    .iter()
                    .flat_map(|list| list.pub_ids.iter().map(PublicKeys::xor_name))
                ...
                let valid_sigs = self
                    .signatures
                    .keys()
                    .filter(|pub_id| valid_names.contains(&pub_id.xor_name()))
                    .count();
...
impl RoutingMessage {
    ...
    pub fn to_signature(&self, signing_key: &SecretKeys) -> Result<DirectMessage, RoutingError> {
...
pub enum MessageContent {
    ...
    RelocateResponse {
        ...
        section: (Prefix<XorName>, BTreeSet<PublicKeys>),
        /// The message's unique identifier.
        message_id: MessageId,
    },
    ...
    OtherSectionMerge(BTreeSet<PublicKeys>, u64),
    ...
    CandidateApproval {
        ...
        new_client_auth: Authority<XorName>,
        /// The `PublicKeys`s of all routing table contacts shared by the nodes in our section.
        sections: SectionMap,
    },
    ...
    NodeApproval {
        /// The routing table shared by the nodes in our group, including the `PublicKeys`s of our
        /// contacts.
        sections: SectionMap,
    },
}
...
impl Debug for DirectMessage {
    fn fmt(&self, formatter: &mut Formatter) -> fmt::Result {
        ...
        match *self {
            ...
            TunnelRequest(ref pub_id) => write!(formatter, "TunnelRequest({:?})", pub_id),
            TunnelSuccess(ref pub_id) => write!(formatter, "TunnelSuccess({:?})", pub_id),
            TunnelSelect(ref pub_id) => write!(formatter, "TunnelSelect({:?})", pub_id),
            TunnelClosed(ref pub_id) => write!(formatter, "TunnelClosed({:?})", pub_id),
            TunnelDisconnect(ref pub_id) => write!(formatter, "TunnelDisconnect({:?})", pub_id),
            ResourceProof {
                ref seed,
                ref target_size,
                ref difficulty,
    ...
    fn fmt(&self, formatter: &mut Formatter) -> fmt::Result {
        ...
        match *self {
            ...
            ExpectCandidate {
                ...
                formatter,
                "ExpectCandidate {{ {:?}, {:?}, {:?} }}",
                old_pub_id, old_client_auth, message_id
            ),
            ConnectionInfoRequest {
                ref pub_id,
                ref msg_id,
            ...
            AcceptAsCandidate {
                ...
                formatter,
                "AcceptAsCandidate {{ {:?}, {:?}, {:?}, {:?} }}",
                old_pub_id, old_client_auth, target_interval, message_id
            ),
            CandidateApproval {
                ref new_pub_id,
                ref new_client_auth,
                ...
                new_pub_id, new_client_auth, sections
            ),
...
mod tests {
    ...
    fn signed_message_check_integrity() {
        ...
        let secret_id = SecretKeys::new();
        let routing_message = RoutingMessage {
            src: Authority::Client {
                client_pub_id: secret_id.public_keys().clone(),
        ...
        let signed_message_result =
            SignedMessage::new(routing_message.clone(), &secret_id, senders);
        ...
        let secret_id = SecretKeys::new();
        let bytes_to_sign = unwrap!(serialise(&(&routing_message, secret_id.public_keys())));
        let signature = secret_id.sign_detached(&bytes_to_sign);

        signed_message.signatures =
            iter::once((secret_id.public_keys().clone(), signature)).collect();
    ...
    fn msg_signatures() {
        ...
        let secret_id_0 = SecretKeys::new();
        let prefix = Prefix::new(0, secret_id_0.public_keys().xor_name());
        let secret_id_1 = SecretKeys::new();
        let secret_id_2 = SecretKeys::new();
        let irrelevant_secret_id = SecretKeys::new();
        ...
        let src_sections = vec![SectionList::from(
            prefix,
            vec![
                secret_id_0.public_keys().clone(),
                secret_id_1.public_keys().clone(),
                secret_id_2.public_keys().clone(),
            ],
        )];
        ...
        let irrelevant_sig = match unwrap!(
            signed_msg
                .routing_message()
                .to_signature(&irrelevant_secret_id,)
        ) {
            DirectMessage::MessageSignature(_, sig) => {
                signed_msg.add_signature(irrelevant_secret_id.public_keys().clone(), sig.clone());
                sig
            }
        ...
        match unwrap!(signed_msg.routing_message().to_signature(&secret_id_1)) {
            DirectMessage::MessageSignature(hash, sig) => {
                ...
                signed_msg.add_signature(secret_id_1.public_keys().clone(), sig);
            }
            msg => panic!("Unexpected message: {:?}", msg),
        }
        let bad_sig = secret_id_2.sign_detached(&[]);
        signed_msg.add_signature(secret_id_2.public_keys().clone(), bad_sig);
        ...
        assert!(
            !signed_msg
                .signatures
                .contains_key(secret_id_2.public_keys())
        );

        // Check an irrelevant signature can't be added.
        signed_msg.add_signature(irrelevant_secret_id.public_keys().clone(), irrelevant_sig);
        assert_eq!(signed_msg.signatures.len(), 2);
        assert!(
            !signed_msg
                .signatures
    ...
    fn hop_message_verify() {
        ...
        let pub_id = full_id.public_keys().clone();
        let hop_message_result =
            HopMessage::new(signed_message.clone(), 0, BTreeSet::new(), &full_id);

        let hop_message = unwrap!(hop_message_result);

        assert_eq!(signed_message, hop_message.content);

        assert!(hop_message.verify(&pub_id).is_ok());

        let random_pub_id = SecretKeys::new().public_keys().clone();
        assert!(hop_message.verify(&random_pub_id).is_err());
    }
