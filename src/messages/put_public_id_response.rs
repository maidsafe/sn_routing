...
mod test {
    ...
    #[test]
    fn put_public_id_response_serialisation() {
        let obj_before : PutPublicIdResponse = Random::generate_random();

        let mut e = cbor::Encoder::from_memory();
        e.encode(&[&obj_before]).unwrap();

        let mut d = cbor::Decoder::from_bytes(e.as_bytes());
        let obj_after: PutPublicIdResponse = d.decode().next().unwrap().unwrap();
