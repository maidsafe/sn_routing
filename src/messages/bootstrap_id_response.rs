...
/// When firing the response, the sender needs to put all its eps in the response so the receiver side can recognize
...
impl Decodable for BootstrapIdResponse {
  fn decode<D: Decoder>(d: &mut D)->Result<BootstrapIdResponse, D::Error> {
    ...
    Ok(BootstrapIdResponse { public_pmid: public_pmid, endpoints: endpoints })
  }
}
