...
use rand::Rng;
use routing::{
    Authority, Client, ClientError, Event, EventStream, MessageId, MutableData, Node,
    PublicKeysExt, Request, Response, SecretKeys, Value, XorName, Xorable, MIN_SECTION_SIZE,
};
...
fn gen_mutable_data<R: Rng>(full_id: &SecretKeys, rng: &mut R) -> MutableData {
    let tag = 10_000;
    ...
    let owner_pubkey = full_id.public_keys().public_sign_key();
...
fn core() {
...

    {
        ...
        let client_key = client.full_id().public_keys().public_sign_key();
