...
#[test]
// This test requires the executable simple_key_value_store is present at the same place of the test get executed
// also it depends on the printout. if such printout is changed / muted, this test needs to be updated
fn executable_test() {
    let mut processes = Vec::new();
    let num_of_nodes = 8;
    // the first vault must be run in zero_membrane mode
    println!("---------- starting node 0 --------------");
    processes.push(match Command::new("./target/debug/examples/simple_key_value_store")
        .arg("-f").stdout(Stdio::piped()).spawn() {
    ...
    for i in 1..num_of_nodes {
        ...
        processes.push(match Command::new("./target/debug/examples/simple_key_value_store")
            .arg("-n").stdout(Stdio::piped()).spawn() {
                    Err(why) => panic!("couldn't spawn maidsafe_vault: {}", why.description()),
                    Ok(process) => process,
                });
